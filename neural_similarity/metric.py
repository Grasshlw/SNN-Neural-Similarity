...
class CCAMetric(SimilarityMetric):
    ...
    def _cca(self, x, y):
        ...
        sigmaxx = covariance[:x_num, :x_num]
        sigmaxy = covariance[:x_num, x_num:]
        sigmayx = covariance[x_num:, :x_num]
        sigmayy = covariance[x_num:, x_num:]

        x_max = np.max(np.abs(sigmaxx))
        y_max = np.max(np.abs(sigmayy))
        sigmaxx /= x_max
        sigmayy /= y_max
        sigmaxy /= np.sqrt(x_max * y_max)
        sigmayx /= np.sqrt(x_max * y_max)

        sigmaxx_inv = np.linalg.pinv(sigmaxx)
        sigmayy_inv = np.linalg.pinv(sigmayy)

        sigmaxx_sqrt_inv = matrix_sqrt(sigmaxx_inv)
        sigmayy_sqrt_inv = matrix_sqrt(sigmayy_inv)

        M = np.dot(sigmaxx_sqrt_inv, np.dot(sigmaxy, sigmayy_sqrt_inv))

        u, s, v = np.linalg.svd(M)
        s = np.abs(s)

        x_ = np.dot(np.dot(u.T, sigmaxx_sqrt_inv), x)
        y_ = np.dot(np.dot(v, sigmayy_sqrt_inv), y)
...
class CCATMetric(CCAMetric):
    def score(self, model_data, neural_data):
        model_data = model_data.transpose(0, 2, 1)
        N1, T1, D1 = model_data.shape
        N2, T2, D2 = neural_data.shape
        model_data = model_data.reshape((-1, D1))
        neural_data = neural_data.reshape((-1, D2))
        ...
        model_lowd = ((model_lowd.reshape((N1, T1, -1))).transpose((1, 0, 2))).reshape((T1, -1))
        neural_lowd = ((neural_lowd.reshape((N2, T2, -1))).transpose((1, 0, 2))).reshape((T2, -1))
...
class RSAMetric(SimilarityMetric): 
    def score(self, model_data, neural_data):
        ...
        model_RDM = 1 - self._pearson_correlation_coefficient(model_data, model_data, matrix=True)
        neural_RDM = 1 - self._pearson_correlation_coefficient(neural_data, neural_data, matrix=True)
...
class RDMMetric(SimilarityMetric):
    def __call__(self, data):
        RDM = 1 - self._pearson_correlation_coefficient(data, data, matrix=True)
        return RDM
...
class RegMetric(SimilarityMetric):
    ...
    def score(self, model_data, neural_data):
        ...
        r = self._pearson_correlation_coefficient(neural_pred, neural_data)
        return np.mean(r)
